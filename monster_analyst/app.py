...
from monster_analyst.src.helpers import WINDOWS_11_CSS, load_custom_metrics, save_custom_metrics, strip_timezone_for_excel, downloadable_dataframe
from monster_analyst.src.data_importer import (
    import_csv, import_excel, import_json, import_google_sheet,
    normalize_columns, detect_column_types, coerce_column_types,
    auto_import,
)
from monster_analyst.src.metric_catalog import CATEGORIES, METRICS, METRICS_BY_CATEGORY, get_metrics_for_available_columns
from monster_analyst.src.formula_engine import apply_formula_to_df, evaluate_formula, validate_formula, bulk_apply_metrics

logging.basicConfig(level=logging.INFO)
log = logging.getLogger("MonsterAnalyst")


def _init_monster_state():
    """Initialize session state for the Monster Analyst data hub."""
...
def render():
    ...
    with tab_import:
        ...
        if st.button("Import Meta Dashboard Output", key="btn_meta_import"):
            if meta_folder and os.path.isdir(meta_folder):
                try:
                    from monster_analyst.src.data_importer import import_meta_dashboard_output
    ...
    with tab_link:
        ...
        if len(ds_names) < 2:
            ...
            from monster_analyst.src.data_linker import suggest_join_columns, join_datasets, join_preview as _join_preview
                ...
                if st.button("Validate Formula", key="btn_validate"):
                    if custom_formula:
                        ...
                        if error:
                            ...
                            from monster_analyst.src.formula_engine import extract_variables
